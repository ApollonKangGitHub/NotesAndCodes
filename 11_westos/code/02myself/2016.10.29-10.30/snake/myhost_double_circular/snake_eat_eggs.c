...
void fb_clear(unsigned long * addr)
{
	int i,j;
	for(i=UP_CHEEK;i<DOWN_CHEEK;i++)
		for(j=LEFT_CHEEK;j<RIGHT_CHEEK;j++)
			*(addr + i*XRES + j) = RGB888(0,0,0);
}
...
void CreateCheek(unsigned long * addr)
{
	...
	for(i = UP_CHEEK - WIDTH_WALL; i < DOWN_CHEEK + WIDTH_WALL; i++)
		for(j = LEFT_CHEEK - WIDTH_WALL; j < LEFT_CHEEK; j++)
			*(addr + i*XRES + j) = RGB888(0, 0X3F,0X1F );/*画左边一竖*/
#endif
#if 1
	for(i = UP_CHEEK - WIDTH_WALL; i < DOWN_CHEEK + WIDTH_WALL; i++)
		for(j = RIGHT_CHEEK; j < RIGHT_CHEEK + WIDTH_WALL; j++)
			*(addr + i*XRES + j) = RGB888(0, 0X3F, 0X1F);/*画右边一竖*/
#endif
#if 1
	for(i = UP_CHEEK - WIDTH_WALL; i < UP_CHEEK; i++)
		for(j = LEFT_CHEEK; j<RIGHT_CHEEK;j++)
			*(addr + i*XRES + j) = RGB888(0, 0X3F, 0X1F);/*画上边一横*/
#endif
#if 1
	for(i = DOWN_CHEEK; i < DOWN_CHEEK + WIDTH_WALL; i++)
		for(j = LEFT_CHEEK; j < RIGHT_CHEEK; j++)
			*(addr + i*XRES + j) = RGB888(0, 0X3F, 0X1F);/*画下边一横*/
#endif
}
	...
	do{
		for(i = p->y; i < p->y + WIDTH; i++)
			for(j = p->x; j < p->x + WIDTH; j++)/*根据顶点坐标（p->x,p->y）与节点宽WIDTH确定填色范围*/
				*(addr + i*XRES + j) = RGB888(0X1F,0X3F,0);/*填充色为黄色*/
		p = p->pNext;
	}while(p && p != pHead);
}
...
SNAKE * NewEgg(unsigned long * addr)
{
	...
	x = (rand()%(RIGHT_CHEEK - LEFT_CHEEK - WIDTH_WALL) + LEFT_CHEEK) / 5  * 5;
	y = (rand()%(DOWN_CHEEK - UP_CHEEK) + UP_CHEEK - WIDTH_WALL)/ 5 * 5;
//	x = r	and() % (RIGTH_CHEEK - LEFT_CHEEK) + LEFT_CHEEK;
//	y = rand() % (DOWN_CHEEK - UP_CHEEK) + UP_CHEEK;
...
{
	SNAKE * pHead = NewNode(250,250);
	SNAKE * pBody = NewNode(255,250);
	SNAKE * pTail = NewNode(260,250);/*为蛇初始化创建三个节点（蛇首、身、尾）*/
